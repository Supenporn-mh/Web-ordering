...
<template>
  <div class="min-h-screen bg-[#f8fcfc] font-sans pb-40">
    ...
    <div class="px-5 py-3 flex items-center bg-white border-b border-gray-200 sticky top-0 z-20 shadow-sm shadow-teal-900/5">
      ...
      <h1 class="text-[18px] font-medium text-gray-800 mx-auto tracking-tight">รายการออเดอร์ที่สั่ง</h1>
    </div>

    <!-- Empty State -->
    <div v-if="cartItems.length === 0" class="flex flex-col items-center justify-center pt-32 px-5 pb-10 text-center animate-fade-in">
      <div class="w-24 h-24 bg-gray-100 rounded-[24px] flex items-center justify-center mb-6">
        <svg class="w-10 h-10 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
      </div>
      <h2 class="text-[20px] font-bold text-gray-800 mb-2 tracking-tight">ไม่มีรายการในตะกร้า</h2>
      <p class="text-gray-500 font-medium text-[15px] mb-8 max-w-[260px] leading-relaxed">ตะกร้าของคุณยังว่างเปล่า กลับไปเลือกเมนูอร่อยๆ กันเถอะ!</p>
      <button @click="router.push({ name: 'menu' })" class="px-8 py-3.5 rounded-[22px] bg-[#228085] text-white font-semibold text-[16px] shadow-lg shadow-teal-500/20 active:scale-95 transition-all">กลับไปหน้าเมนู</button>
    </div>

    <div v-else class="px-4 pt-6 pb-10 space-y-4">
      <!-- Location Card (Uniform rounded-[24px]) -->
      <div class="bg-white rounded-[24px] p-5 shadow-sm border border-gray-200">
        <!-- Header Row -->
        <div class="flex justify-between items-center mb-3">
          <h3 class="text-[18px] font-medium text-gray-900 tracking-tight">สถานที่จัดส่ง</h3>
          <button @click="router.push({ name: 'checkout' })" class="text-[#228085] font-semibold text-[16px] tracking-tight">แก้ไข</button>
        </div>
        ...
        <div class="flex items-start gap-3.5 mb-4">
          ...
          <div class="flex-1">
             <template v-if="deliveryOption === 'delivery'">
               <p class="font-medium text-gray-900 text-[18px] leading-snug">ห้อง : {{ roomNumber }}</p>
               <p class="font-medium text-gray-900 text-[18px] leading-snug">HN : {{ hnNumber }}</p>
             </template>
             <template v-else>
               <p class="font-medium text-gray-900 text-[18px] leading-snug">รับที่ร้าน : {{ pickupTime }}</p>
             </template>
          </div>
        </div>
        ...
        <div class="bg-[#f0f4f4] p-4 rounded-[18px] border border-gray-200">
          <p class="text-[14px] font-medium text-gray-600 mb-1 uppercase tracking-wider">หมายเหตุการสั่ง</p>
          <p class="text-[18px] font-medium text-gray-900 leading-tight">
      ...
      <div class="bg-white rounded-[24px] p-5 shadow-sm border border-gray-200">
        <h2 class="text-[18px] font-medium text-gray-800 mb-4 text-center tracking-tight border-b border-gray-200 pb-2">รายการออเดอร์</h2>
          ...
          <div v-for="(item, index) in cartItems" :key="item.id" class="flex gap-4">
            ...
            <div class="flex-1 min-w-0">
               <div class="flex justify-between items-start mb-1">
                  <h4 class="font-medium text-gray-800 text-[16px] leading-tight pr-1 tracking-tight truncate">{{ item.name }}</h4>
                  <div class="flex items-center gap-2">
                    <button @click="openEditModal(item, index)" class="px-3 py-1.5 rounded-full bg-white border border-gray-200 shadow-sm text-[12px] text-[#228085] font-semibold hover:bg-[#228085] hover:text-white transition-all leading-tight">แก้ไข</button>
               ...
               <div class="space-y-0.5">
                  <p v-if="index === 0" class="text-[14px] font-semibold text-gray-800">เพิ่มผัก (+฿15)</p>
                  <p v-if="item.desc" class="text-[14px] text-[#228085] font-medium line-clamp-1">
                    <span class="text-gray-600 font-medium">หมายเหตุ :</span> {{ item.desc }}
                  </p>
               </div>
               <div class="flex justify-between items-center mt-3">
                  <div class="flex items-center gap-4 bg-[#f0f4f4] p-1.5 rounded-full border border-gray-300 shadow-sm">
                     <button @click="updateQuantity(index, -1)" class="w-8 h-8 rounded-full bg-white border border-gray-300 flex items-center justify-center text-[#228085] font-semibold active:scale-90 shadow-sm">−</button>
                  <span class="text-[16px] font-semibold text-gray-800 w-5 text-center">{{ item.qty || item.quantity }}</span>
                  <button @click="updateQuantity(index, 1)" class="w-8 h-8 rounded-full bg-[#228085] border border-[#228085] flex items-center justify-center text-white font-semibold active:scale-90 shadow-md">+</button>
               </div>
               <span class="text-[16px] font-semibold text-[#228085]">฿{{ (item.price * (item.qty || item.quantity)).toLocaleString() }}</span>
            </div>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white rounded-[24px] p-5 shadow-sm border border-gray-200">
        <div class="flex items-center gap-3 mb-3">
           ...
           <h3 class="text-[16px] font-medium text-gray-800">คูปองส่วนลด / รหัสโปรโมชั่น</h3>
        </div>
        <div class="flex gap-3">
          ...
          />
          <button @click="applyPromo" style="line-height:1.6; padding-top:10px; padding-bottom:10px;" class="px-6 rounded-[18px] bg-[#228085] text-white font-semibold text-[15px] flex items-center justify-center shrink-0">ใช้สิทธิ์</button>
        </div>
      </div>

      <!-- Payment Summary Card (Uniform rounded-[24px]) -->
      <div class="bg-white rounded-[24px] p-5 shadow-sm border border-gray-200">
        <h2 class="text-[18px] font-medium text-gray-800 mb-4 text-center tracking-tight">สรุปยอดชำระ</h2>
        <div class="space-y-4 pb-5 border-b border-gray-200 mb-5">
          ...
          <div class="flex justify-between items-center text-[16px] font-medium text-gray-800">
             <span>ภาษี (7%)</span>
             <span class="font-semibold">฿{{ vat.toLocaleString() }}</span>
          </div>
          <div class="flex justify-between items-center text-[16px] font-medium text-gray-800">
             <span>Service Charge 10%</span>
             <span class="font-semibold">฿{{ serviceCharge.toLocaleString() }}</span>
          </div>
        </div>
        <div class="flex justify-between items-center">
           <span class="text-[18px] font-medium text-gray-900">ยอดรวมสุทธิ</span>
           <span class="text-[#228085] font-semibold text-[22px]">฿{{ total.toLocaleString() }}</span>
        </div>
      </div>
    </div>
      ...
      <div class="bg-white w-full max-w-sm rounded-[24px] overflow-hidden relative shadow-2xl animate-fade-in">
        <div class="p-7 flex flex-col max-h-[85vh]">
          <div class="flex justify-between items-start mb-6">
             <h2 class="text-[18px] font-medium text-gray-800 pr-10 line-clamp-1 truncate">{{ selectedItem?.name }}</h2>
          ...
          <div class="mb-7">
             <h3 class="text-[14px] font-medium text-gray-500 uppercase tracking-widest mb-3 ml-1">หมายเหตุเพิ่มเติม</h3>
          ...
          <div class="pt-6 border-t border-gray-100 flex flex-col gap-6">
             <div class="flex items-center justify-between">
                <span class="text-[16px] font-medium text-gray-700">เลือกจำนวน</span>
                <div class="flex items-center gap-5 bg-[#f0f4f4] p-1 rounded-full border border-gray-200">
                   <button @click="editQuantity > 1 ? editQuantity-- : null" class="w-10 h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-[#228085] font-semibold shadow-sm active:scale-95">−</button>
                   <span class="text-[16px] font-semibold text-gray-800">{{ editQuantity }}</span>
                   <button @click="editQuantity++" class="w-10 h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-[#228085] font-semibold shadow-sm active:scale-95">+</button>
                </div>
             </div>
             <button @click="saveEdit" class="w-full py-4.5 rounded-[24px] bg-[#228085] text-white font-semibold text-[18px] shadow-xl shadow-teal-500/20 active:scale-[0.98]">
    ...
    <div v-if="cartItems.length > 0" class="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[375px] bg-white/95 backdrop-blur-md border-t border-gray-100 p-6 pb-10 z-30 shadow-[0_-8px_30px_rgba(0,0,0,0.08)]">
      <button 
        @click="handleConfirmOrder" 
        :disabled="cartItems.length === 0"
        class="w-full py-5 rounded-[24px] bg-[#228085] text-white font-semibold text-[18px] shadow-xl shadow-teal-500/20 active:scale-[0.98] disabled:opacity-50 transition-all"
      >
        ยืนยันการสั่งซื้อ
      </button>
    </div>
